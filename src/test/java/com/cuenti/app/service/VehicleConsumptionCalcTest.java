...
class VehicleConsumptionCalcTest {
    ...
    void partialFillAccumulatesUntilNextFullTank() {
        ...
        BigDecimal[] attributed = VehicleReportService.computeDerivedValues(entries);

        assertThat(partial.getConsumption()).isNull();
        // 20 + 35 liters over 1000 km = 5.50 L/100km
        assertThat(second.getConsumption()).isEqualByComparingTo("5.50");
    ...
    void withoutFullTankFlags_everyFillMeasures() {
        ...
        VehicleReportService.computeDerivedValues(List.of(a, b));

        // 30 liters over 500 km = 6.00 L/100km (classic fill-to-fill)
        assertThat(b.getConsumption()).isEqualByComparingTo("6.00");
        assertThat(a.getConsumption()).isNull(); // first fill has no baseline
    }
    ...
    void pricePerLiterComputedPerEntry() {
        ...
        VehicleReportService.computeDerivedValues(List.of(a));
        assertThat(a.getPricePerLiter()).isEqualByComparingTo("1.550");
    }
}
