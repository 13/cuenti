package com.cuenti.homebanking.views.dashboard;
...
@PageTitle("Dashboard")
@Route("")
@Menu(order = 0, icon = LineAwesomeIconUrl.TACHOMETER_ALT_SOLID)
@PermitAll
public class DashboardView extends VerticalLayout {
    ...
    private void createAssetPerformanceSection() {
        ...
        Span totalLabel = new Span("Total Portfolio");
    ...
    private Div createGainLossDisplay(BigDecimal gainLoss, BigDecimal percent) {
        ...
        percentSpan.getStyle().set("font-size", "12px").set("color", color);
    ...
    private void createCharts(List<Transaction> transactions) {
        ...
        HorizontalLayout timeHeader = new HorizontalLayout(new H4("Cash Flow Analysis"));
        ...
        legend.add(createLegendItem("Revenue", "var(--lumo-success-color)"),
                createLegendItem("Spending", "var(--lumo-error-color)"));
        ...
        totalChartContent.add(new H4("Top Spending Categories"));
    ...
    private void renderTimeChart(Div container, List<Transaction> transactions, String range) {
        ...
        if ("Daily".equals(range)) {
            chartData.put(today.format(java.time.format.DateTimeFormatter.ofPattern("dd.MM")), new BigDecimal[]{BigDecimal.ZERO, BigDecimal.ZERO});
        } else if ("Weekly".equals(range)) {
            chartData.put("W" + today.get(java.time.temporal.IsoFields.WEEK_OF_WEEK_BASED_YEAR), new BigDecimal[]{BigDecimal.ZERO, BigDecimal.ZERO});
        } else if ("Monthly".equals(range)) {
            chartData.put(today.getMonth().getDisplayName(TextStyle.SHORT, userLocale), new BigDecimal[]{BigDecimal.ZERO, BigDecimal.ZERO});
        } else {
        ...
        for (Transaction t : transactions) {
            ...
            if ("Daily".equals(range) && td.isEqual(today)) {
                label = td.format(java.time.format.DateTimeFormatter.ofPattern("dd.MM"));
            } else if ("Weekly".equals(range) &&
    ...
    private void createAccountList(List<Account> accounts) {
        ...
        Div content = new Div();
        content.getStyle().set("padding", "var(--lumo-space-m)");
        content.add(new H4(getTranslation("dashboard.accounts_overview")));
